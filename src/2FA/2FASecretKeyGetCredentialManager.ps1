...
#############################################################################################################################################
# Flag para indicar um fechamento controlado
$script:NormalExit = $false

#############################################################################################################################################
# Função para ser executada no fechamento
function OnClosing {
    if (-not $script:NormalExit) { 
        Write-Host "Tentativa de fechamento forçado detectada. Reiniciando o script..." -ForegroundColor Red
        Start-Process -FilePath "pwsh" -ArgumentList "-WindowStyle Hidden  -NoProfileLoadTime -NoProfile -NonInteractive -NoLogo -STA -Login -NoExit -File `"$PSCommandPath`""
        Start-Sleep -Seconds 1
    }
}

# Registra o evento
$OnCloseEvent = Register-EngineEvent -SourceIdentifier "PowerShell.Exiting" -Action { OnClosing }
...
try {
    ...
    while ($true) {
        ...
        try {
            ...
            [System.Windows.Forms.Application]::Run($form) 2>&1>$null
            #############################################################################################################################################
